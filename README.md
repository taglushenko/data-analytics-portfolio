# Портфолио аналитика данных

Привет! Меня зовут Татьяна Глушенко.

Я продуктовый аналитик с опытом в бизнес-анализе и Data Science.тВ этом репозитории собраны мои проекты по анализу данных, маркетинговой аналитике, продуктовым метрикам и A/B-тестированию.


## Проекты

### Python

- [Исследование рынка общественного питания Москвы](./09_moscow-food-market-analysis)
- [Анализ воронки продаж и A/A/B-теста мобильного приложения](./08_mobile-app-funnel-ab-test)
- [Приоритизация гипотез и анализ результатов A/B-теста интернет-магазина](./07_ab-test-business-decision-making)
- [Анализ эффективности маркетинговых инвестиций Procrastinate Pro+](./06_procrastinate-unit-economics)
- [Анализ рынка компьютерных игр](./05_video-game-market-analysis)
- [Статистический анализ данных сервиса GoFast](./04_gofast-statistical-analysis)
- [Исследование рынка недвижимости Санкт-Петербурга](./03_real-estate-market-analysis)
- [Исследование надёжности заёмщиков](./02_borrower-reliability-analysis)
- [Исследование поведения пользователей Яндекс Музыки](./01_yandex_music_user_behavior)

### Google Sheets

- [Анализ выручки, расходов и маркетинговых кампаний салона Waves](./11_google-sheets-business-analysis)

### SQL

- [Анализ данных StackOverflow с помощью SQL](./10_stackoverflow-sql-analysis)

### Дашборды

- [Анализ TED Talks в Tableau](./12_ted-talks-tableau-dashboard)

## Инструменты

- Python
- SQL
- Pandas
- NumPy
- SciPy
- Matplotlib
- Seaborn
- Folium
- Google Sheets
- Excel
- Tableau

## Навыки

- Исследовательский анализ данных (EDA)
- Очистка и подготовка данных
- Визуализация данных
- Статистический анализ
- Проверка гипотез
- A/B-тестирование
- Геоаналитика
- Продуктовая аналитика
- Маркетинговая аналитика
- Когортный анализ
- Юнит-экономика
- Анализ воронок
- Анализ пользовательского поведения
- Построение дашбордов
